...
job_id varchar(10),
department_id int,
constraint emp_start_date_id_pk primary key (employee_id,start_date),
constraint job_id_fk foreign key (job_id) references jobs (job_id) on delete cascade on update cascade,
...
alter table jobs alter column region_y type varchar(60)
...
Insert into JOBS (JOB_ID,JOB_TITLE,MIN_SALARY,MAX_SALARY) values ('AD_VP','AdministrationVicePresident','15000','30000');
Insert into JOBS (JOB_ID,JOB_TITLE,MIN_SALARY,MAX_SALARY) values ('AD_ASST','AdministrationAssistant','3000','6000');
...
update countries set country_name = 'indonesia',region_id = 3  where country_id = '99' 
...
select country_name from countries where country_name 
like 'da%' or country_name like 'ba%' or country_name like 'oa%' or country_name like 'kev%'

select * from departments 
where location_id in (select location_id from locations l, countries c where l.country_id = c.country_id )

insert into countries values(99,'banjaran')

alter table employees add constraint department_id_fk foreign key (department_id) references departments (department_id) on delete cascade on update cascade
